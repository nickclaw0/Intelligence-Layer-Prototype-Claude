...
- **Installable skill bundles on Drive.** Three zips sit in the Drive project folder for installing into Claude.ai: `velorixa-intelligence-layer-skill.zip` (the wiki query skill = `connector-skill/SKILL.md`), `velorixa-avalere-pptx-skill.zip`, and `velorixa-avalere-docx-skill.zip`. The two generator bundles embed a copy of the engine (`build_deck.py` / `build_doc.py`) and the pinned template under `assets/`, so they are self-contained for code execution. They are snapshots: if a generator engine, template, or SKILL.md changes in the repo, re-export the affected bundle (stage the skill folder with `assets/<template>` alongside, zip with SKILL.md at the root) so the installed skill does not drift. The engines resolve the template from `assets/` (bundle) or `../_assets/` (repo) automatically.
