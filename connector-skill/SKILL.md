...
3. **Write a spec JSON** whose every factual claim carries a source id. The engine renders citations inline as `[src:id]`.
   - Doc spec: `{blocks:[...]}`. A block is a styled paragraph `{style, text, citations?}`; nest with `List Bullet 2..5`; add a branded table with `{"type":"table","style":"1 by 1 Grey with Orange Rule","header":true,"rows":[[...],[...]]}`. Use tables for anything comparative; lead with Title/Subtitle and structure with Headings rather than a wall of bullets.
   ...
   - **Make the deck dense and specific, not flat.** The number-one quality failure is an on-brand deck that comes out half-empty or vague. Compose with the care of a freehand deck, then express it through the template: pick content-rich layouts where the material suits them, fill *every* slot describe-layout lists (no empty columns or stat boxes), give each box a short header plus supporting points, and use enough slides to tell the story. Be **specific** — real names and roles, exact phrases, concrete figures, the named decision and who made it — not vague summaries. The template supplies the brand; you supply rich, sourced, specific content for every slot, to the standard of a strong hand-made deck.
